...
window.collider = new (function(){
	...
	function raycast(pos, dir, lines) {
	...
	//var some things out here for performance reasons
	var ePos = vec2.create();
	var eDir = vec2.create();
	var dummy = vec2.create();

	function sweepEllipse(pos, dir, lines, eDimensions) {
		var t=1;

		var oPos = pos;
		var oDir = dir;

		var dim = 1/eDimensions[0];

		var pos = vec2.scale(ePos, pos, dim); //need to rescale position to move into ellipsoid space
		var dir = vec2.scale(eDir, dir, dim);
		...
		for (var i=0; i<lines.length; i++) {
			...
			if (!(t0>1 || t1<0)) { //we intersect with the line at some point during the time!
				...
				var pt = [];
				if (embedded) {
					vec2.sub(pt, pos, vec2.scale(dummy, l.normal, dist));
				} else {
					vec2.add(pt, pos, vec2.scale(dummy, dir, newT))
				...
				for (var j=1; j<=2; j++) {
					var vert = vec2.sub(dummy, pos, l["p"+j]);
	...
	function modLine(inL, eDim) {
		return {
			p1: vec2.scale([], inL.p1, eDim),
			p2: vec2.scale([], inL.p2, eDim),
			normal: inL.normal
		}
	}
})();