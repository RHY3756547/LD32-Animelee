...
window.SvgColParse = function(svg){
	...
	t.svg = svg;
	t.lines = [];
	t.base = [Infinity, Infinity];
	t.limit = [-Infinity, -Infinity]
	parse();
			...
			for (var i=0; i<s.length; i++) {
				...
				if (reading) {
					if (code == "z" || code == "m" || code == "zm") {
						...
						if (last[0] < t.base[0]) t.base[0] = last[0];
						if (last[1] < t.base[1]) t.base[1] = last[1];
						if (last[0] > t.limit[0]) t.limit[0] = last[0];
						if (last[1] > t.limit[1]) t.limit[1] = last[1];
